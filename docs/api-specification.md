...
이메일 파라미터를 전달한 경우 등록·수정과 동일한 형식 규칙을 적용한다. 빈 문자열, 공백 또는 형식이 잘못된 이메일은 `400 Bad Request`이며, 유효하지만 존재하지 않는 이메일은 `200 OK`와 `[]`다. 이메일 파라미터를 생략하면 기존의 선택 필터 동작을 유지한다.
...
이메일 삭제 조건도 등록·수정과 동일한 형식 규칙을 적용한다. 빈 문자열, 공백 또는 형식이 잘못된 이메일은 `400 Bad Request`이며, 유효하지만 존재하지 않는 이메일은 `404 Not Found`다.
