...
Service
  AddressBookService
  service/dto/CustomerSearchRequest
...
```text
src/main/java/com/hyundai/test/address/service/dto/CustomerSearchRequest.java
```

Controller의 HTTP 쿼리 파라미터는 기존과 동일하다. HTTP 검색 조건과 Service 입력 필드가 완전히 같아 `@ModelAttribute`로 `CustomerSearchRequest`에 직접 바인딩한다. 이 객체는 Service 입력 계약이므로 `controller.dto`에 두지 않고, 원시 정렬 문자열을 포함해 핵심 도메인도 아니므로 애플리케이션 서비스 입력 경계인 `service.dto`에 둔다.
...
### 5.5 DTO Builder와 생성자 주입 정리

- `CustomerRequest`, `CustomerResponse`, `CustomerChangeResponse`, `ErrorResponse`, `CustomerSearchRequest` record에 Lombok `@Builder` 적용
- DTO를 직접 만드는 프로덕션 코드와 테스트는 Builder 사용
- `AddressBookController`, `AddressBookService`에 `@RequiredArgsConstructor` 적용
- 설정값 변환 또는 생성자 파라미터 `@Value`가 필요한 클래스는 명시적 생성자 유지

record는 불변 데이터, 접근자, 값 비교를 제공하고 Builder는 이름 기반 생성 API를 추가한다. 둘은 경쟁하는 선택지가 아니며 필드가 많거나 같은 타입이 반복되는 경계 DTO에서 함께 사용할 수 있다.

### 5.6 Gradle Java 17 Toolchain 적용
