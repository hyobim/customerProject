...
Windows에서는 `JAVA_HOME`이 실제 Java 17 설치 경로를 가리켜야 합니다. 현재 한글 프로젝트 경로에서도 Java 17과 Gradle 8.4로 빌드 및 테스트가 통과했습니다.
...
- 원본 교체에 실패하면 방금 생성한 백업으로 기존 원본 복구를 시도합니다.
- 성공·실패 여부와 관계없이 저장용 임시 파일을 정리합니다.
...
도메인 검증, 저장소 동시성, 서비스, CSV 경계값과 저장 실패, 실제 Spring 종료 생명주기, MockMvc API 테스트를 포함합니다. 2026-06-20 기준 테스트 33개가 현재 한글 경로에서 모두 통과했습니다.
